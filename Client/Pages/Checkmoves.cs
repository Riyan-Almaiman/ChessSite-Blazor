...

{
    public class CheckMoves
    {
        ...
        public static bool IsCheck(string select, int[] selectindex, int[] tileindex, string[,] BoardLayout, List<string> pieces)
        {
            var boardlayout = new string[8, 8];
                ...
                for (int i = 0; i <= 7; i++)
                {
                    ...
                    for (int j = 0; j <= 7; j++)
                    {
                        ...
                        boardlayout[i, j] = BoardLayout[i, j];
            ...
            boardlayout[tileindex[0], tileindex[1]] = select;
            boardlayout[selectindex[0], selectindex[1]]="  ";
            if (select.ToUpper() == select)
            {

                foreach (string piece in boardlayout)
                {
                    if (piece != "  " && piece.ToUpper() != piece)
                    {

                        if (IsValidMove(piece, indexOfPiece(piece), indexOfPiece("K1"), boardlayout, pieces)) { return false; }
